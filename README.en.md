...
| [oss-research](oss-research/SKILL.md) | Start from either end: turn a broad need into a scored GitHub shortlist with explicit rejections and a selection gate, or deeply research a named repo by profiling its author, auditing hooks before installation, running it sandboxed, verifying a core mechanism, and delivering an evidence-based report. | "Find open-source projects for this need", "shortlist GitHub repos", "research this repo" |
...
The skills chain together: `oss-research` finds the right project and reads it deeply → `code-renew` renovates it → `oss-publish` ships your result → `plain-write` polishes anything meant for human eyes.
...
Then just talk to Claude — "find GitHub projects for this need", "research this repo", or "humanize this email" will trigger the matching skill. You can also invoke explicitly: "use oss-research on X".
